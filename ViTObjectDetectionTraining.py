...
def main():
    ...
    batch_size = 64
    random_seed = 42
    num_classes = 81  # 80 COCO classes 
    num_boxes = 100  
    num_epochs = 5
    learning_rate = 0.005
    weight_decay = 0.0001
   ...
   # Initialize COCOParser
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (images, targets )in enumerate(tqdm(train_loader)):
            ...
            # Process the model output to get the selected labels, boxes, and scores
            #all_selected_labels, all_selected_boxes, all_selected_scores = process_model_output(class_logits, box_coordinates, threshold=0.5)

           # Reshape box_coordinates to [batch_size, num_predicted_boxes, 4]
            #box_coordinates = box_coordinates.view(-1, num_predicted_boxes, 4)
            # Reshape class_logits to match the dimensions of box_coordinates
            #class_logits = class_logits.view(-1, num_predicted_boxes, num_classes) 

            # Compute loss
            loss = compute_loss(class_logits, box_coordinates, targets, coco_parser)
            print(f"loss:", loss)
            print(f"loss/length(train loader):", loss/len(train_loader))
        ...
        torch.save({
            ...
            'loss': train_loss,
        }, f"/home/ps332/myViT/object_detection_model_epoch_{epoch}.pth")
